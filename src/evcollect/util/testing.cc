...
int UnitTest::main(int argc, const char* argv[]) {
  ...
  if (flags.isSet("log-level")) {
    std::string value = flags.getString("log-level");
    StringUtil::toUpper(&value);
    Logger::get()->setMinimumLogLevel(strToLogLevel(value));
  }

  if (flags.isSet("log-target")) {
    std::string logTargetStr = flags.getString("log-target");
    if (logTargetStr == "stderr") {
      Logger::logToStderr("evcollectd");
    } else if (logTargetStr == "syslog") {
      Logger::logToSyslog("evcollectd");
    } else {
      printf("Unknown --log-target specified (\"%s\")\n", logTargetStr.c_str());
      return 1;
    }
  }

  filter_ = flags.getString("filter");
  repeats_ = flags.getInt("repeat");
  randomize_ = flags.isSet("randomize");
  printProgress_ = !flags.isSet("no-progress");
